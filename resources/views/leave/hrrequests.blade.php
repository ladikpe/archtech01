              ...
              <table class="table table striped datatable">
                ...
                <tbody>
                  <tr>
                    ...
                    <td><span
                        class=" tag tag-outline  {{$leave_request->priority==0?'tag-success':($leave_request->priority==1?'tag-warning':'tag-danger')}}">{{$leave_request->priority==0?'normal':($leave_request->priority==1?'medium':'high')}}</span>
                    </td>
                    <td>{{$leave_request->reason}}</td>
                    <td>{{$leave_request->length}}</td>
                    <td><span
                        class=" tag   {{$leave_request->status==0?'tag-warning':($leave_request->status==1?'tag-success':'tag-danger')}}">{{$leave_request->status==0?'pending':($leave_request->status==1?'approved':'rejected')}}</span>
                    </td>
                      ...
                      <div class="btn-group" role="group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="exampleIconDropdown1" role="menu">
                          <a data-id="{{ $leave_request->id }}" style="cursor:pointer;"
                            class="dropdown-item view-approval" id="{{$leave_request->id}}"
                            onclick="viewRequestApproval(this.id)"><i class="fa fa-eye"
                              aria-hidden="true"></i>&nbsp;View Approval</a>
                          ...
                          <a style="cursor:pointer;" class="dropdown-item" id=""
                            href="{{url('leave/download?leave_request_id='.$leave_request->id)}}"><i
                              class="fa fa-download" aria-hidden="true"></i>&nbsp;Download Document</a>
                          @endif
                        </div>
                      </div>
                    </td>
          ...
          <div class="panel-heading">
            ...
            <div class="panel-actions">
              ...
              <select class="form-control" onchange="changeYear(this.value)">
                @for($i=$first_year;$i<=date('Y');$i++) @if(request()->year!='')
