   ...
   <modal name="kpi-form" :height="'269px'">
    ...
    <div slot="top-right" style="text-align: right;padding: 7px;">
      ...
      <form @submit.prevent="validateInput">
        <div class="col-xs-12">
          <h4>{{ edited? 'Save':'Add' }} Organizational KPI</h4>
        </div>
        ...
        <div class="col-xs-12">
          ...
          <input type="text" v-model="data.percentage" class="form-control" />
        </div>
   ...
   <!-- modal stop -->
    ...
    <div class="col-md-12">
      ...
      <div class="col-xs-12" align="right">
           <a href="" class="btn btn-sm btn-success" data-toggle="modal" @click.prevent="edited=false;$modal.show('kpi-form')">Add Organizational KPI</a>
      </div>


      <!-- start -->
      <div class="col-xs-4">
        <h4 style="margin:0;">Organizational KPIs</h4>
      </div>
  ...
  methods: {
    ...
    getKpiDepartments() {
      if (this.data.dept_id && this.data.kpi_frequency_id) {
        this.startAjax();
        fetch(
          "/api/get-kpi-organization/" + this.data.dept_id + "/" + this.data.kpi_frequency_id
        )
          .then(res => res.json())
          .then(res => {
            this.stopAjax();
    ...
    doSave() {
      this.startAjax();
      fetch("/api/update-kpi-organization/" + this.data.id, {
        method: "POST",
        body: this.getInput(),
    ...
    doCreate() {
      this.startAjax();
      fetch("/api/store-kpi-organization", {
        method: "POST",
        body: this.getInput(),
        headers: this.headers
      })
    ...
    doRemove(data) {
      ...
      if (confirm("Do you want to confirm this action?")) {
        this.startAjax();
        fetch("/api/remove-kpi-organization/" + data.id, {
          method: "POST",
          body: this.getInput(),
          headers: this.headers
        })
