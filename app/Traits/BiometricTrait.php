    ...
    public function saveAttendance($data)
    {
        ...
        if ($user) {
            ...
            if ($data['status_id'] == 0 || $data['status_id'] > 1) {//clock in
                ...
                //fresh clockin
                AttendanceDetail::updateOrCreate(['attendance_id' => $attendance->id, 'clock_in' => date('H:i:s', strtotime($data['time'])),'clock_in_lat'=>$lat,'clock_in_long'=>$long]);
                ...
                if ($ad) {  //a clockin exists
                    if ($ad->clock_out == '') {    //a clockin exists but no clockout, create a clockout
                        $ad->update(['clock_out' => date('H:i:s', strtotime($data['time'])),'clock_out_lat'=>$lat,'clock_out_long'=>$long]);
                        ...
                        AttendanceDetail::updateOrCreate(['attendance_id' => $attendance->id, 'clock_in' => date('H:i:s', strtotime($data['time'])),
                            'clock_out' => date('H:i:s', strtotime($data['time'])),'clock_out_lat'=>$lat,'clock_out_long'=>$long]);
                    }
