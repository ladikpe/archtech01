...
trait LeaveTrait
{
    ...
    public function myRequests(Request $request)
    {
        ...
        $pending_leave_requests = Auth::user()->leave_requests()->where('status', 0)->whereYear('start_date', date('Y'))->get();
        $leave_requests = Auth::user()->leave_requests()->whereYear('start_date', date('Y'))->orderBy('created_at', 'desc')->get();

        $leaves = Leave::orderBy('id')->get();
    ...
    public function approvals(Request $request)
    {
        ...
        $approval_history = $user->leave_approvals()->orderBy('created_at', 'desc')->get();

        return view('leave.approvals', compact('user_approvals', 'approval_history', 'role_approvals', 'group_approvals', 'dr_approvals'));
    }
    ...
    {
        $leave_requests = LeaveRequest::where('replacement_id', \Auth::user()->id)->where('relieve_approved', 0)->orderBy('id', 'desc')->get();
