	...
	private function download(Request $request){
	    ...
	    try{
            if($request->user()->role->permissions->contains('constant', 'download_document')){
                ...
                $path = \App\Document::where('id',$request->id)->first();
                $path2 = \App\Document::where('id',$request->id)->update(['last_mod_id'=>$request->user()->id]);
                //Put Condition for permission here
                return response()->download(storage_path("app/$path->document_link"));
            }
            else{
                return 'You do not have permission to perform this action, Press the back button on your browser to continue';

            }
        }
        catch (\Exception $ex){
	        return 'Document Does Not Exist, Press the back button on your browser to continue';
        }
