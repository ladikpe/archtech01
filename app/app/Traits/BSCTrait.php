...
private function downloadTemplate(Request $request){
                           ...
                           $template=['Perspective','Business Goal','Weighting','Measure/KPI','Source','Lower Target','Mid-Target','Upper-Target'];
                           ...
                           foreach ($datas[0] as $data) {
                           	...
                           	if ($data[0]) {
			            			$metric=BscMetric::where('name',$data[0])->first();
			            			 $det_detail=BscDetDetail::create(['bsc_det_id'=>$det->id,'bsc_metric_id'=>$metric->id,'business_goal'=>$data[1],'measure'=>$data[3],'source'=>$data[4],'lower'=>$data[5],'mid'=>$data[6],'upper'=>$data[7],'weighting'=>$data[2]*100]);
...
public function get_bsc_grade_performance_category(Request $request)
    {
        return $bsc_grade_performance_category = \App\BscGradePerformanceCategory::where('id', $request->performance_category_id)->with('grades')->first();
    }
    ...
    public function save_bsc_grade_performance_category(Request $request)
    {
        try {
            ...
            $bsc_grade_performance_category = \App\BscGradePerformanceCategory::updateOrCreate(['id' => $request->performance_category_id], ['name' => $request->name, ]);
	...
	public function saveBehavioralSubMetric(Request $request)
	{
		...
		\App\BehavioralSubMetric::updateOrCreate(['id'=>$request->bsm_id],['objective'=>$request->objective,'measure'=>$request->measure,'weighting'=>$request->weighting,'low_target'=>$request->low_target,'mid_target'=>$request->mid_target,'upper_target'=>$request->upper_target,'status'=>$request->status,'company_id'=>companyId()]);
		return 'success';
	}
