...
class AttendanceController extends Controller
{
    ...
    public function exportShiftSchedules(Request $request)
    {
        ...
        elseif(Auth::user()->role->manages == 'all'){
            $users = User::whereIn('status',[1,0])->where('company_id', $company_id)->get();
        }
        else{
            $users = User::whereIn('status',[1,0])->where('company_id', $company_id)->get();
        }
        ...
        $this->exportToExcel2($users, $dates, $view, $name);
    }
    ...
    public function downloadShiftUploadTemplate(Request $request)
    {
        ...
        $company_id = companyId();
        $users = DB::table('users')->where('status','1')->where('company_id',$company_id)->select('emp_num as Staff Id', 'name as Employee Name')->get()->toArray();
        ...
        return $this->exportshiftexcel('template', ['users' => $users, 'shifts' => $shifts], $heading);
    }
    ...
    public function appScheduleShift(Request $request){
        ...
        return view('attendance.new.app_schedule_shift',compact('date','departments','users','dates','users_shifts','shifts'));
    }
