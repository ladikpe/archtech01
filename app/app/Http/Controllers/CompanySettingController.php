...
class CompanySettingController extends Controller
{
  ...
  //       }
	...
	//end job functions
		//divsions functions
	public function divisions($department_id)
	{
		$company=Company::find(companyId());
		$department=Department::find($department_id);
		$divisions=$department->divisions;
		$users=$company->users;
		return view('settings.companysettings.division',compact('divisions','company','department','users'));
	}
	public function saveDivision(Request $request)
	{
	   // $division=Division::where(['name'=>$request->name,'department_id'=>$request->department_id])->first();
	   // if($division){
	   //     return response()->json(['success'],200);
	   // }
		 Division::updateOrCreate(['id'=>$request->division_id],['name'=>$request->name,'manager_id'=>$request->manager_id,'department_id'=>$request->department_id]);
